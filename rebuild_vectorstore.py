...
def extract_metadata_from_markdown(content: str, headers: dict) -> dict:
    ...
    metadata = {"sección": "General", "subsección": "N/A", "categoria": "N/A"}

    # Mapeo de headers a metadata
    if "H1" in headers:
        metadata["sección"] = headers["H1"]
    if "H2" in headers:
        metadata["subsección"] = headers["H2"]
    if "H3" in headers:
        metadata["categoria"] = headers["H3"]

    # Extraer palabras clave del contenido
    keywords = ["cuenta", "crédito", "tarjeta", "seguro", "inversión"]
...
def build_vectorstore():
    ...
    for md_file in docs_path.glob("*.md"):
        ...
        # Split adicional por tamaño
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=800,  # Tamaño optimizado para documentos densos
    ...
    print(f"✅ {len(all_docs)} chunks creados")
    ...
    # Añadir documentos con batching para memoria limitada
    batch_size = 100
    for i in range(0, len(all_docs), batch_size):
        batch = all_docs[i:i + batch_size]
        vectorstore.add_documents(batch)
        print(f"  → Procesados {min(i + batch_size, len(all_docs))} chunks...")

    print(f"✅ Vectorstore creado en {settings.VECTORSTORE_PATH}")
